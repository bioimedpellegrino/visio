...
key = None #globale
breakout_flag=False #globale
def keyboard_scan():
    global key #global perche key viene modificata nella funzione
    while True:
        key = None
        key = readchar()
        sleep(0.01)
        if breakout_flag==True: #non serve global perche breakout_flag viene solo letta
            break
